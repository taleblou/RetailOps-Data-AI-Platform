...
Supports the connector shopify layer inside the modular repository architecture. Standardizes external-system interaction for the connector shopify workflow.
...
- `main.py` — Provides implementation support for the connector shopify workflow.
- `connector.py` — Implements adapter logic for the connector shopify integration surface.
- `schemas.py` — Defines schemas for the connector shopify data contracts.
- `__init__.py` — Defines the modules.connector_shopify package surface and package-level exports.
- `Dockerfile` — Container build definition for this component.

## Interfaces and data contracts

- Main types: ShopifyConnector, ShopifyConnectorConfig.
...
- Important dependencies: Standard library only, __future__, json, typing, urllib.error, urllib.parse, urllib.request, time, pydantic.
...
- `tests/connector_shopify/` automated checks for this module when present
