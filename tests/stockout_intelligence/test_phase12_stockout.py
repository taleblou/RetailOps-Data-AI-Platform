...
from modules.stockout_intelligence.service import (
    ...
    run_phase12_stockout_risk,
)


def _write_phase12_upload(tmp_path: Path) -> tuple[str, Path, Path]:
    uploads_dir = tmp_path / "uploads"
    uploads_dir.mkdir(parents=True, exist_ok=True)
    artifact_dir = tmp_path / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    upload_id = "phase12_upload"
...
def test_run_phase12_stockout_builds_ranked_predictions(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase12_upload(tmp_path)
...
def test_phase12_router_returns_stockout_list_and_detail(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase12_upload(tmp_path)
