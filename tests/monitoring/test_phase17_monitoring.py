...
def _write_phase17_upload(tmp_path: Path) -> tuple[str, Path]:
    uploads_dir = tmp_path / "uploads"
    uploads_dir.mkdir(parents=True, exist_ok=True)
    upload_id = "phase17_upload"
...
def _prepare_phase17_artifacts(tmp_path: Path) -> dict[str, Path | str]:
    upload_id, uploads_dir = _write_phase17_upload(tmp_path)
...
def test_run_phase17_monitoring_builds_governance_artifact(tmp_path: Path) -> None:
    paths = _prepare_phase17_artifacts(tmp_path)
    artifact = get_or_create_phase17_monitoring_artifact(
...
def test_phase17_manual_override_logging_keeps_feedback(tmp_path: Path) -> None:
    paths = _prepare_phase17_artifacts(tmp_path)
    entry = log_phase17_manual_override(
        upload_id=str(paths["upload_id"]),
    ...
    summary = get_phase17_override_summary(
        upload_id=str(paths["upload_id"]),
        override_dir=Path(paths["override_dir"]),
    )
...
def test_phase17_router_returns_monitoring_summary_and_override_log(tmp_path: Path) -> None:
    paths = _prepare_phase17_artifacts(tmp_path)
