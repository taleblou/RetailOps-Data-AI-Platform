...
# File:         test_forecasting_batch.py
# Path:         tests/forecasting/test_forecasting_batch.py
#
...
# Notes:
#   - Main types: None.
#   - Key APIs: test_run_batch_forecast_builds_summary, test_get_product_forecast_reads_product_from_artifact, test_forecasting_forecast_router_returns_summary_and_product
...
from modules.forecasting.service import get_product_forecast, run_batch_forecast


def _write_forecasting_upload(tmp_path: Path) -> tuple[str, Path, Path]:
    uploads_dir = tmp_path / "uploads"
    uploads_dir.mkdir(parents=True, exist_ok=True)
    artifact_dir = tmp_path / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    upload_id = "forecasting_upload"
...
def test_run_batch_forecast_builds_summary(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_forecasting_upload(tmp_path)
...
def test_forecasting_forecast_router_returns_summary_and_product(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_forecasting_upload(tmp_path)
