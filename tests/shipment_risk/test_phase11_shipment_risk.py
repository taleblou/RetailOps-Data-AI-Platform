...
from modules.shipment_risk.service import get_open_order_prediction, run_phase11_shipment_risk


def _write_phase11_upload(tmp_path: Path) -> tuple[str, Path, Path]:
    uploads_dir = tmp_path / "uploads"
    uploads_dir.mkdir(parents=True, exist_ok=True)
    artifact_dir = tmp_path / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    upload_id = "phase11_upload"
...
def test_run_phase11_shipment_risk_builds_open_order_artifact(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase11_upload(tmp_path)
...
def test_phase11_router_returns_open_orders_and_prediction(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase11_upload(tmp_path)
