...
from modules.returns_intelligence.service import (
    ...
    run_phase14_returns_intelligence,
)


def _write_phase14_upload(tmp_path: Path) -> tuple[str, Path, Path]:
    uploads_dir = tmp_path / "uploads"
    uploads_dir.mkdir(parents=True, exist_ok=True)
    artifact_dir = tmp_path / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    upload_id = "phase14_upload"
...
def test_run_phase14_returns_builds_scores_and_risky_products(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase14_upload(tmp_path)
...
def test_phase14_router_returns_orders_products_and_detail(tmp_path: Path) -> None:
    upload_id, uploads_dir, artifact_dir = _write_phase14_upload(tmp_path)
