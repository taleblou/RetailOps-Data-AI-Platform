...
# Module:       tests.setup
# File:         test_setup_wizard.py
# Path:         tests/setup/test_setup_wizard.py
#
# Summary:      Contains automated tests for the setup workflows and behaviors.
# Purpose:      Validates setup behavior and protects the repository against regressions.
# Scope:        test
# Status:       stable
#
...
# Notes:
#   - Main types: None.
#   - Key APIs: test_setup_setup_api_flow, test_setup_wizard_html_pages
#   - Dependencies: __future__, pathlib, fastapi.testclient, core.api.main, core.ingestion.base.repository
...
def test_setup_setup_api_flow(tmp_path: Path, monkeypatch) -> None:
...
def test_setup_wizard_html_pages(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    assert "RetailOps Setup wizard Setup Wizard" in home_response.text
