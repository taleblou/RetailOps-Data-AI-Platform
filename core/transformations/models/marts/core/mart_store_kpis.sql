...
select
    ...
    {{ safe_divide('coalesce(sh.delayed_shipment_count, 0)', 'nullif(coalesce(sh.shipment_count, 0), 0)') }} as delayed_shipment_rate,
    coalesce(i.sku_count, 0) as inventory_sku_count,
    coalesce(i.out_of_stock_sku_count, 0) as out_of_stock_sku_count,
    {{ safe_divide('coalesce(i.out_of_stock_sku_count, 0)', 'nullif(coalesce(i.sku_count, 0), 0)') }} as out_of_stock_rate,
    i.avg_available_qty
from sales s
